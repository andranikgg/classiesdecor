    ...
    public function addLanguageUrlRules()
    {
        if (!$this->_languagesRuleAdded && $this->languagesEnabled())
        {
           ...
           // $rules[$langRoute . '/'] = '/';
            ...
            $rules[$langRoute . '/<controller:\w+>/<action:\w+>/<id:\d+>'] = '<controller>/<action>';
            $rules[$langRoute . '/<controller:\w+>/<action:\w+>/<id:\d+>/<id2:\d+>'] = '<controller>/<action>';


            $rules[$langRoute . '/module/<module:\w+>/<controller:\w+>/<action:\w+>'] = '<module>/<controller>/<action>';
            $rules[$langRoute . '/<module:\w+>/<controller:\w+>/<action:\w+>/<id:\d+>'] = '<module>/<controller>/<action>/<id>';
    ...
    public function getLanguageLinks($htmlOptions = array(),$params = array(), $ampersand = '&', $labels = array())
    {
        ...
        foreach ($urls as $language => $url) {
            if(Yii::app()->language != $language)
                $links[] = array('url'=>$url, 'label'=>$language);
        }

        return $links;
    }

    public function getLanguageLinksFooter($htmlOptions = array(),$params = array(), $ampersand = '&', $labels = array())
    {
        $links = array();
        $labels = array_merge($this->languages,$labels);
