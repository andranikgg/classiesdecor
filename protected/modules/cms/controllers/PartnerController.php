...
class PartnerController extends SecureController
{
    ...
    public function actionUpload()
    {
        try {
            ...
            $size = getimagesize($_SERVER['DOCUMENT_ROOT'] . Yii::app()->baseUrl . '/images/partner/temp/' . $result['filename']);

            if ($size[0] > 800) {
                $newHeight = 800*$size[1]/$size[0];
                Yii::app()->ih
                    ->load($_SERVER['DOCUMENT_ROOT'] . Yii::app()->baseUrl . '/images/partner/temp/' . $result['filename'])
                    ->resize(800, $newHeight, false)
                    ->save($_SERVER['DOCUMENT_ROOT'] . Yii::app()->baseUrl . '/images/partner/temp/' . $result['filename']);
            }
            $size = getimagesize($_SERVER['DOCUMENT_ROOT'] . Yii::app()->baseUrl . '/images/partner/temp/' . $result['filename']);
            if ($size[1] > 600) {
